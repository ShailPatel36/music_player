...
export default class MusicPlayer {
  ...
  armDisplacement(time) {
    ...
    const rotationUnit =
      startPositionOnTheDisc + displacement * parseFloat(time);

    armElement.style.transform = `rotate(${rotationUnit}deg)`;
  }

  setMusic() {
    const srcMusic = `./assets/sounds/${this.musics[this.current].musicFile}`;
    const srcAlbum = `./assets/images/${this.musics[this.current].albumFile}`;
  ...
  prev() {
    ...
    if (this.shuffle) {
      this.alreadyPlayed =
        this.alreadyPlayed - 1 < 0 ? listLength : this.alreadyPlayed - 1;
